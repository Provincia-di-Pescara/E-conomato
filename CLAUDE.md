...
- **Roles**: `user`, `funzionario`, `magazziniere`, `admin`, plus `economo` (Cassa Economale module, in roadmap). Stored in `utenti.ruolo`; assigned from LDAP groups at login. Precedence in `resolveRole()`: `admin > magazziniere > economo > funzionario > user`. Admin bypass is enforced in `requireRole`, not in the DB.
